"""Test script: Manually swap L/R for camera 00_03 before Triangulation."""
...
def main():
    ...
    for ci, cn in enumerate(cams):
        ...
        if img_path.exists():
            ...
            if p:
                ...
                # MANUALLY SWAP 00_03 (Back Camera)
                if cn == "00_03":
                    kpts, scores = swap_lr(kpts, scores)

        all_kpts2d.append(kpts)
        all_scores.append(scores)

    all_kpts2d = np.array([all_kpts2d]) # (1, 4, 17, 2)
    all_scores = np.array([all_scores]) # (1, 4, 17)
    ...
    print(f"Frame 150 MPJPE after L/R swap on 00_03: {errs.mean():.1f} mm")
    ...
    plot_skel(pred3d, 'red', 'Triangulated (Swapped 00_03)')
    
    ax.legend()
    ax.set_xlim(-200, -100)
    ax.set_ylim(-50, 50)
    ax.set_zlim(0, 200)
    out = ROOT / "swapped_viz.png"
